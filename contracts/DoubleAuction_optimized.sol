...
// Importing MathForDoubleAuction : calculate average/quick sort descending/quick sort ascending
...
contract DoubleAuction is MathForDoubleAuction {
  ...
  struct Clearing {
    uint32 clearingQuantity;
    uint32 clearingPrice;
    uint32 clearingType; // marginal_seller = 1, marginal_buyer = 2,  exact = 3, null = 4
  }

  address public market; // who deployed this contract 
  uint32[] _consumptionPrices; 
  uint32[] _generationPrices;
  mapping(uint32 => uint32) consumptionBids;
  mapping(uint32 => uint32) generationBids;
  mapping(address => Bid)  consumptionaddress;
  mapping(address => Bid)  generationaddress;
  Clearing public clearingInfo;
  uint clearingpricebuyleft;
  uint clearingpricesell;
  ...
  // buyers set bid
  function BuyerBid(address _buyer,uint32 _quantity, uint32 _price) payable public {
   ...
   //sellers set bid
  function SellerBid(address _seller,uint32 _quantity, uint32 _price) public{
  ...
  function computeClearing() private {
    bool check = false;
    uint32 demand_quantity = 0;
    uint32 supply_quantity = 0;
    uint32 buy_quantity = 0;
    uint32 sell_quantity = 0;
    uint32 i = 0;  
    uint32 j = 0;
    
    //sort arrays, consumer's bid descending, producer's ascending
    quickSortDescending(_consumptionPrices, 0, uint32(_consumptionPrices.length - 1));
    quickSortAscending(_generationPrices, 0, uint32(_generationPrices.length - 1));
  ...
  function ClearAll() public {
    for (uint cleanConsumptionIndex = 0; cleanConsumptionIndex < _consumptionPrices.length; cleanConsumptionIndex++){
      uint32 consPrice = _consumptionPrices[cleanConsumptionIndex];
    ...
    for (uint cleanGenerationIndex = 0; cleanGenerationIndex < _generationPrices.length; cleanGenerationIndex++){
      uint32 genPrice = _generationPrices[cleanGenerationIndex];
  ...
  function MakePayment()public payable {
    ...
    if (consumptionaddress[recipiant].exist==true){ // is a buyer
     if(consumptionaddress[recipiant].price < clearingInfo.clearingPrice){
       value = consumptionaddress[recipiant].price * consumptionaddress[recipiant].quantity;
     }
     else if(consumptionaddress[recipiant].price > clearingInfo.clearingPrice){
       value = (consumptionaddress[recipiant].price-clearingInfo.clearingPrice) * consumptionaddress[recipiant].quantity;
     }
     else{
       value = clearingInfo.clearingPrice * clearingpricebuyleft;
     }
    }
    // I use 2 if here instead of if-else, so that a buyer can also be a seller
    if (generationaddress[recipiant].exist==true){// is a seller
      if(generationaddress[recipiant].price < clearingInfo.clearingPrice){
       value = clearingInfo.clearingPrice * generationaddress[recipiant].quantity;
     }
     else if(generationaddress[recipiant].price == clearingInfo.clearingPrice){
       value = clearingInfo.clearingPrice * clearingpricesell;
     }
     else{
       value = 0;
     }
